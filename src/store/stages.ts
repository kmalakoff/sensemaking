...
export interface StageRecorder {
  ...
  take(totalMs: number, txMs: number, parseWorkerMs?: number): Stages;
}
...
export function stageRecorder(featureNames: readonly string[] = []): StageRecorder {
  ...
  return {
    ...
    take(totalMs, txMs, parseWorkerMs = 0) {
      const out: Record<string, number> = {};
      for (const [stage, ms] of spans) out[stage] = round(ms);
      return { version: STAGES_VERSION, totalMs: round(totalMs), txMs: round(txMs), parseWorkerMs: round(parseWorkerMs), spans: out };
    },
  };
}
