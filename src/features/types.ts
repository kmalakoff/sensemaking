...
export interface Feature {
  ...
  // Pure, per file: raw is the full file, body the prose after frontmatter, search the normalized
  // title/summary, data the frontmatter, cfg the resolved config, blocks the body's parse tree (parsed once by parseFile, shared).
  ...
  // Once per reconcile (not per file) with every path whose rows need clearing: vanished files,
  // plus reparsed ones that already had rows (a feature diffing its own stale rows in store() filters this to delta.vanished).
  ...
  // Per-file opt-out (currently only embed, via FileStat.embed): absent means the feature applies
  // to every file it's otherwise on for; checked before extract/store so such docs get no rows at all.
