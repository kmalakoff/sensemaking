...
3. **Obsidian parity, when the release touches parsing** (tags, links, sections, fences, frontmatter): `node benchmark/oracle.mjs obsidian-hub-b11036f9 .tmp/cache/obsidian-hub-b11036f9` diffs sense's tags table against Obsidian's own metadata cache -- the reference implementation, so drift cannot ship silently. The target is the repo's own pinned benchmark corpus; one-time setup per machine: open that folder in Obsidian via "Open folder as vault" (the registration is path-based and survives `.tmp` regeneration; the `.obsidian/` it writes lives in a throwaway cache dir). The script needs Obsidian running and stores nothing -- dump and index live in temp and are deleted. A second run against a real working vault (ask the maintainer for name and path) is the shape that has caught bugs the public corpus missed. Zero differing files passes; anything else is adjudicated line by line, never averaged away.
...
5. Update BENCHMARKING.md: results table, scale table, **capabilities column for the new version**. Write down which movements were judged noise, so the next reader does not re-hunt them.
