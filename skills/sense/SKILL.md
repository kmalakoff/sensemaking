...
- SQLite's `datetime()` accepts a narrower spelling than ISO 8601 does, so sense normalizes the difference at index time: a colonless offset (`-0800`) and a space date/time separator are rewritten to the form SQLite parses. Punctuation only, never a timezone conversion, so `substr(d,1,10)` is still the local date.
