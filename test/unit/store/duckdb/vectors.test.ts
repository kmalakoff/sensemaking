...
async function makeDb(dims = DIMS) {
  ...
  let duckdb: Awaited<ReturnType<typeof instance.connect>> | undefined;
  try {
...
async function withDb<T>(fn: (duckdb: Awaited<ReturnType<typeof makeDb>>['duckdb'], conn: Connection) => Promise<T>): Promise<T> {
  const { instance, duckdb, conn } = await makeDb();
  try {
    return await fn(duckdb, conn);
  } finally {
    try {
      duckdb.disconnectSync();
    } finally {
...
describe('scanCandidates (duckdb)', () => {
  it('orders separated mixed-direction vectors by descending cosine similarity', async () => {
    await withDb(async (duckdb, conn) => {
      ...
      const result = await scanCandidates(duckdb, fixture.query, DIMS, 10);
      assert.deepEqual(
        result.map((r) => r.path),
  ...
  it('narrows candidates to the allowed set', async () => {
    await withDb(async (duckdb, conn) => {
      ...
      const result = await scanCandidates(duckdb, fixture.query, DIMS, 10, new Set(['b.md']));
      assert.deepEqual(
        result.map((r) => r.path),
        ['b.md']
      );
  ...
  it('an empty allowed set matches nothing, same as sqlite', async () => {
    await withDb(async (duckdb, conn) => {
      await insertPending(conn, 'a.md', 0);
      await writeVectorBatch(duckdb, conn, DIMS, [{ path: 'a.md', chunk: 0, ...quantizeVector(full(1, 0)) }]);
...
describe('scanSimilar (duckdb)', () => {
  ...
  it('returns [] when the target note has no embedded chunks', async () => {
    await withDb(async (duckdb, conn) => {
      ...
      await writeVectorBatch(duckdb, conn, DIMS, [{ path: 'other.md', chunk: 0, ...quantizeVector(fixture.exact) }]);
