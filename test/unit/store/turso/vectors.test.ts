...
async function makeConn(dims = DIMS): Promise<Connection> {
...
describe('scanCandidates (turso)', () => {
  it('orders results by descending cosine similarity', async () => {
    ...
    const result = await scanCandidates(conn, full(1, 0), DIMS, 10);
    assert.deepEqual(
      result.map((r) => r.path),
  ...
  it('narrows candidates to the allowed set', async () => {
    ...
    const result = await scanCandidates(conn, full(1, 0), DIMS, 10, new Set(['b.md']));
    assert.deepEqual(
      result.map((r) => r.path),
      ['b.md']
    );
  ...
  it('an empty allowed set matches nothing, same as sqlite', async () => {
    const conn = await makeConn();
    await insertPending(conn, 'a.md', 0);
    await writeVectorBatch(conn, DIMS, [{ path: 'a.md', chunk: 0, ...quantize(full(1, 0)) }]);
...
describe('scanSimilar (turso)', () => {
  ...
  it('returns [] when the target note has no embedded chunks', async () => {
    ...
    await writeVectorBatch(conn, DIMS, [{ path: 'other.md', chunk: 0, ...quantize(full(1, 0)) }]);
