...
describe('presets: file coverage', () => {
  it('a two-preset tree indexes the union of both, and preset_files records each assignment', async () => {
    ...
    const rows = (await (await store.prepare('SELECT path FROM frontmatter ORDER BY path')).all()) as Array<{ path: string }>;
    assert.deepEqual(
      rows.map((r) => r.path),
      ['raw/b.md', 'wiki/a.md']
    );

    const coverage = (await (await store.prepare('SELECT preset, path FROM preset_files ORDER BY preset, path')).all()) as Array<{ preset: string; path: string }>;
  ...
  it('overlapping presets are not an error: a file matched by two presets is indexed once, both assignments recorded', async () => {
    const baseDir = tmpTree();
    ...
    const rows = (await (await store.prepare('SELECT COUNT(*) AS n FROM frontmatter')).get()) as { n: number };
    assert.equal(rows.n, 1, 'the file is indexed once regardless of how many presets match it');

    const coverage = ((await (await store.prepare('SELECT preset FROM preset_files WHERE path = ? ORDER BY preset')).all('shared/x.md')) as Array<{ preset: string }>).map((r) => r.preset);
    assert.deepEqual(coverage, ['alpha', 'beta', 'default']);
  });

  it('a preset whose globs match nothing is valid, and coverage stays empty for it', async () => {
    const baseDir = tmpTree();
    ...
    const empty = (await (await store.prepare('SELECT COUNT(*) AS n FROM preset_files WHERE preset = ?')).get('empty')) as { n: number };
    assert.equal(empty.n, 0);
  });

  it('a file matched by no preset at all is absent from the index entirely', async () => {
    const baseDir = tmpTree();
    writeNote(baseDir, 'wiki/a.md', { frontmatter: { title: 'A' } });
  ...
  it('exclude narrows a preset without needing another preset to resolve an overlap', async () => {
    const baseDir = tmpTree();
  ...
  it('a symlinked directory is not indexed a second time under the link path', async () => {
    const baseDir = tmpTree();
  ...
  it('brace expansion and extglob patterns still resolve in a preset include', async () => {
    const baseDir = tmpTree();
    writeNote(baseDir, 'a.md', { frontmatter: { title: 'A' } });
...
describe('stored paths are POSIX on every platform', () => {
  it('a Windows separator is normalized', async () => {
    assert.equal(toPosixPath('notes\\sub\\a.md', '\\'), 'notes/sub/a.md');
    assert.equal(toPosixPath('a.md', '\\'), 'a.md');
  });

  it('a POSIX platform leaves the path alone, since a backslash is a legal filename character', async () => {
