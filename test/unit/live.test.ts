...
// Every test here talks to a real endpoint, so every test here is gated on a variable in
// .env.test (gitignored): an unconfigured machine and CI skip the file, a maintainer with the
// file runs it before a release. One gate per INTEGRATIONS.md row, so a row's claim is
// reproducible rather than remembered, and a missing gate is why a row cannot be added.
// The mock-server tests in providers.test.ts pin request shape on every push; these pin that
// the shape is still the one the real services speak.
try {
