...
function makeTree(extra: Record<string, unknown> = {}): string {
  const dir = mkdtempSync(join(tmpdir(), 'sense-segment-'));
...
describe('segmentMatch', () => {
  // Verbatim from the validated reference implementation (scratchpad reference.mjs).
...
describe('substring parity: search behaves like grep over unspaced-script text', () => {
  // The docs from the reference implementation's exhaustive validation: an ICU-killer split
  // (东京都政府, where ICU splits differently depending on what follows), a doc with no ICU
  // dictionary entries at all, and mixed kana+kanji.
  ...
  function makeParityTree(): string {
    const dir = mkdtempSync(join(tmpdir(), 'sense-parity-'));
    writeConfig(dir);
...
describe('ranking: mirrored bm25 weights put a title hit above a body hit', () => {
  function makeRankTree(): string {
    const dir = mkdtempSync(join(tmpdir(), 'sense-rank-'));
    writeConfig(dir);
