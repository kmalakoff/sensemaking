...
type GraphStore = Awaited<ReturnType<typeof openTree>>['store'];

async function withGraph<T>(run: (store: GraphStore) => Promise<T>): Promise<T> {
...
describe('findPath', () => {
  ...
  it('same node: a single-element path', async () => {
    await withGraph(async (store) => assert.deepEqual(await findPath(store, 'a.md', 'a.md'), ['a.md']));
  });
  ...
  it('directed: no path against the arrows', async () => {
    await withGraph(async (store) => assert.equal(await findPath(store, 'e.md', 'a.md', { directed: true }), null));
  });
  ...
  it('unreachable: an isolated node returns null', async () => {
    await withGraph(async (store) => assert.equal(await findPath(store, 'a.md', 'o.md'), null));
  });

  it('maxDepth bounds the search: too short to reach', async () => {
    await withGraph(async (store) => assert.equal(await findPath(store, 'a.md', 'e.md', { maxDepth: 2 }), null));
  });
